...
export async function LoadCmdsEmbedData() {
    ...
    logthis(yellow(bold(`[Database] Querying 'Cmds Embed Data' database.`)))
    ...
    const poolpromise = new Promise((err, res) => {
        ...
        CmdEmbed.query(QueryDB, (err, res) => {
            if (err) {
                logthis(red(bold(`[DEBUGGING node-postgres QUERY]:`)))
                console.log(err)
            } else {
                logthis(green(bold(`[Database] Successfully accessed 'cmdsembed`)))
