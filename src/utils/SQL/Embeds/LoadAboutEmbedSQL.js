...
export async function LoadAboutEmbedDatabase() {
    ...
    logthis(yellow(bold(`[postgreSQL] Querying 'aboutembed' database.`)))
    ...
    const poolpromise = new Promise((err, res) => {
        ...
        AboutEmbedPool.query(quer, (err, res) => {
            if (err) {
                logthis(red(bold(`[DEBUGGING node-postgres QUERY]:`)))
                console.log(err)
            } else {
                logthis(green(bold(`[postgreSQL] Successfully accessed 'aboutembed`)))
