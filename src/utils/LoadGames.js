  ...
  export async function LoadGames(HomeAndAwayTeamsObject) {
    ...
    try {
      ...
      const data = await page.$$eval(`#fittPageContainer > div:nth-child(4) > div > div > section > div > div:nth-child(3) > div:nth-child(1) > div > div.flex > div > div.Table__Scroller > table > tbody`, tds => tds.map((td) => {
        ...
        /**
        Below, I transform, store and pass along all the NBA Data we have scrapped. To help understand why things below are done the way they are, you must first understand Puppeteer's response
        ...
        (CompiledGameData) => {
          ...
          //? In this LoadGames function, all the data scrapped returns on indiviudal lines. Turning the string into an array.
          ...
          //! ---------------------------------------- //! Organizing / Storing the NBA Team name's scrapped into objects ---------------------------------------- */
          ...
          /* ------------------------------------------------------------------------------------------------------------------------------------------------ */
          logthis(magentaBright(cborder))
          //? # of Keys in the object that currently should hold all the teams we've scrapped so far.
