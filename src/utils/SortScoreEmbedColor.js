...
import { bold, logthis, green, yellowBright } from '../lib/hercConfig.js';
import {NBATeamHexColors} from '../lib/Storage/NBATeamHexColors.js'
const NBAHexColors = NBATeamHexColors


export function SortScoreEmbedColor(AwScore, HScore, AwayTeamNameUppercase, HomeTeamNameUppercase) {
    logthis(green(bold(`[DEBUGGING]: NBA HEX OBJECT: ${NBAHexColors}`)))
    if (WhoWon(AwScore, HScore, AwayTeamNameUppercase, HomeTeamNameUppercase) == `Away`) {
        logthis(yellowBright(`-----------`))
        console.log(AwScore)
        var winningteam = JSON.stringify(AwayTeamNameUppercase)
        var findQuotes = /['"]+/g;
      ...
      //  logthis(green(bold(`[DEBUGGING]: NBA HEX COLOR FOR SUNS: ${testHex}`)))
        ...
        return embedHex;
    } else if (WhoWon(AwScore, HScore, AwayTeamNameUppercase, HomeTeamNameUppercase) == 'Home') {
        logthis(yellowBright(`-----------`))
        console.log(HScore)
        var winningteam = JSON.stringify(HomeTeamNameUppercase)
        var findQuotes = /['"]+/g;
        var winningteamNoQuotes = winningteam.replace(findQuotes, '')
...
export function WhoWon(AwScore, HScore, AwayTeamNameUppercase, HomeTeamNameUppercase) {
    if (parseInt(AwScore) == parseInt(HScore)) {
        var HTeamWithQuotes = JSON.stringify(HomeTeamNameUppercase)
        var findQuotes = /['"]+/g;
        var HTeamNoQuotes = HTeamWithQuotes.replace(findQuotes, '')
        var AwayTeamWithQuotes = JSON.stringify(HomeTeamNameUppercase)
